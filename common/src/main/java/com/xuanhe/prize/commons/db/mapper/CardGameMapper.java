...
public interface CardGameMapper {
     ...
     */
    @Insert({
        "insert into card_game (title, info, ",
        "starttime, endtime, ",
        "type, status)",
        "values (#{title,jdbcType=VARCHAR}, #{info,jdbcType=VARCHAR}, ",
        "#{starttime,jdbcType=TIMESTAMP}, #{endtime,jdbcType=TIMESTAMP}, ",
        "#{type,jdbcType=TINYINT}, #{status,jdbcType=BIT})"
    })
    ...
    @Select({
        "select",
        "id, title, info, starttime, endtime, type, status",
        "from card_game",
        "where id = #{id,jdbcType=INTEGER}"
    })
    @ResultMap("com.xuanhe.prize.commons.db.mapper.CardGameMapper.BaseResultMap")
    ...
    List<CardGame> selectByNowTime(@Param("nowtime") Date now,@Param("minutelater") Date minuteLater);

    List<CardProductDto> getCardProductDtoByGameId(Integer gameId);
